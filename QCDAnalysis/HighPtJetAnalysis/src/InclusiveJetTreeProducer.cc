       ...
       if (mHltConfig.init(iRun,iSetup,mTriggerResultsTag.process(),changed)) 
         {
            ...
            for(unsigned int i=0;i<mTriggerNames.size();i++)
              { 
                for(int j=0;j<20;j++)
                {
                  std::string tmp_hlt_path = mTriggerNames[i]+"_v"+std::to_string(j);
                  unsigned int hlt_i = mHltConfig.triggerIndex(tmp_hlt_path);
                  if(hlt_i<100)
                  {
                    mTriggerIndex.push_back(hlt_i); 
                    std::cout<< tmp_hlt_path << "  "<<hlt_i <<std::endl;
                    break;
                  }
                } 
            ...
            if (changed) 
              {
                ...
                std::vector<std::string> triggers = mHltConfig.triggerNames();
                std::cout<<"Print nu HLT names"<<std::endl;
                //for(unsigned int i=0;i<mHltConfig.size();i++)
                //{
...
{ 
  ...
  */
      ...
      if (!triggerResultsHandle.isValid())
        {
          ...
          ErrFlag=-1;
          std::cout << "triger empty" << std::endl;
        //  Again we don't want to abort the entire job in this case
        //  throw  cms::Exception("Configuration",errorMessage);
        }
      //std::cout << "triger not empty" << std::endl;
      //std::vector<std::string> triggers = mHltConfig.triggerNames();
      //for(unsigned int i=0;i<mHltConfig.size();i++)
      //{
      ...
      for(unsigned int i=0;i<mTriggerNames.size();i++) 
        {
          ...
          mHLTTrigResults[i].L1prescale=0;
          mHLTTrigResults[i].HLTprescale=0;
	  std::string hlt_v; 
          bool accept=false;
          ...
          try {
              ...
              unsigned int trIndex=mHltConfig.triggerIndex(hlt_v);
              //unsigned int trIndex = mTriggerIndex[i];
            ...
            if (accept) {
              mHLTTrigResults[i].fired=1;
              int l1 = mHltConfig.prescaleValues(event, iSetup, hlt_v).first;
              int hlt = mHltConfig.prescaleValues(event, iSetup, hlt_v).second;
              int hltv = mHltConfig.prescaleValue(event, iSetup, hlt_v);
              if(hlt!=hltv){std::cout << "shit "<<std::endl;}
              mHLTTrigResults[i].L1prescale  = l1;
              mHLTTrigResults[i].HLTprescale = hlt; //mHltConfig.prescaleValue(event, iSetup, hlt_v);
              mHLTTrigResults[i].prescale = l1*hlt; //mHltConfig.prescaleValue(event, iSetup, hlt_v);
            }
          }
        }
    }
  ...
  /* for(unsigned int i=0;i<mTriggerNames.size();i++)
   {
     std::cout << mTriggerNames[i] << endl;
     std::cout << mHLTTrigResults[i].fired << endl;
     std::cout << mHLTTrigResults[i].L1prescale << endl;
     std::cout << mHLTTrigResults[i].HLTprescale << endl;
   }*/ 
...
void InclusiveJetTreeProducer::buildTree() 
{
  ...
  for (unsigned int jname=0;jname<mTriggerNames.size();jname++) {
     ...
     mTree->Branch(branchname,&mHLTTrigResults[jname],"L1prescale/I:HLTprescale/I:prescale/I:fired/I");
     }
